...
- **Archaeological relief** from national LiDAR (0.5 m to 1 m resolution depending on source):
  ...
  > and the DTM interpolates straight through them. Typically (observed, not a
  > guaranteed rule), a 40 cm enclosure wall survives (absorbed into the ground
  > class) while a 1.5 m house ruin vanishes cleanly. No shading computed from
  > the DTM can bring them back. For
...
**Survive the SSH disconnect with `tmux`.** Run directly over SSH and the job dies the moment your connection drops (laptop asleep, PC off). `tmux` keeps it running on the server and lets you reattach later, live progress bar and all:
...
# detach: Ctrl-b then d. Close SSH, shut down your PC.
